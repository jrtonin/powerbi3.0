...

{
  ...
  fPop <- melt(fPop)
  colnames(fPop) <- c("city_IPARDES", "variable", "year", "population")
  fPop$variable <- "population"
  fPop$year <- as.character(fPop$year)
  fPop <- left_join(fPop, fCity, "city_IPARDES")
...
```{r health}
{
  ...
  colnames(dHospi)[1:2] <- c("city_IPARDES", "variable")
  
  # Step 3: Join city code with database
  dHospi <- left_join(dHospi, fCity, by = "city_IPARDES")
  
  #Step 4: rearrange dHospi
  dHospi <- dHospi[, c(15, 1, 2:13)]
...

{
  ...
  # Step 2: Rename columns
  colnames(dImun)[1:2] <- c("city_IPARDES", "variable")
  
  # Step 3: Join city code with database
  dImun <- left_join(dImun, fCity, by = "city_IPARDES")
  
  #Step 4: rearrange dImun
  dImun <- dImun[, c(15, 1, 2:13)]
...

{
  ...
  # Step 2: Rename columns
  colnames(dLeitos)[1:2] <- c("city_IPARDES", "variable")
  
  # Step 3: Join city code with database
  dLeitos <- left_join(dLeitos, fCity, by = "city_IPARDES")
  
  #Step 4: rearrange dLeitos
  dLeitos <- dLeitos[, c(15, 1, 2:13)]
...

{
  ...
  # Step 2: Rename columns
  colnames(dMorta)[1:2] <- c("city_IPARDES", "variable")
  
  # Step 3: Join city code with database
  dMorta <- left_join(dMorta, fCity, by = "city_IPARDES")
  
  #Step 4: rearrange dMorta
  dMorta <- dMorta[, c(15, 1, 2:13)]
...

{
  ...
  # Step 2: Rename columns
  colnames(dProfi)[1:2] <- c("city_IPARDES", "variable")
  
  # Step 3: Join city code with database
  dProfi <- left_join(dProfi, fCity, by = "city_IPARDES")
  
  #Step 4: rearrange dProfi
  dProfi <- dProfi[, c(15, 1, 2:13)]
...

{
  ...
  # Step 2: Rename columns
  colnames(dExpen)[1:2] <- c("city_IPARDES", "variable")
  
  # Step 3: Join city code with database
  dExpen <- left_join(dExpen, fCity, by = "city_IPARDES")
  
  #Step 4: rearrange dExpen
  dExpen <- dExpen[, c(15, 1, 2:13)]
...
{
  # Step 1: Join
  dHealth <- left_join(dHealth, fPop, by = c("city_IPARDES", "year", "city_code7"))
...
{
  # Step 1: Import CSV
  dInfra <- read.csv("~/Profissional/SEPL/PowerBi 3/Banco de Dados/infrastructure/dInfrastructure.csv")
  
  dInfra <- dInfra[, c(7, 2,3,4,5,9)]
  
  # Step 2: Delete Education Rows
  dInfra <- dInfra[!c(dInfra$group == "Instituições de Educação Superior" |
                    dInfra$group == "Polos de Apoio dos Cursos de Educação Superior a Distância") , ]
  
} # Part I: Import dInfra

{
  # Step 1: Create references dataframes
  fInfraGroup <- dInfra[, c(2,3)]
  
  # Step 2: Select Group
  fInfraGroup <- fInfraGroup[ !duplicated(fInfraGroup), ]
  
  # Step 3: Generate Codes
  fInfraGroup <- fInfraGroup %>%
    mutate(variable_code = sequence(n()))
  
  x <- data.frame(group = unique(fInfraGroup$group)) %>%
    mutate(group_code = sequence(n()))
  
  fInfraGroup <- left_join(fInfraGroup, x, "group")
  
} # Part 2: Extract f table
