---
title: "PowerBI3.0"
author: "Equipe PR Produtivo"
date: "10/08/2021"
output: html_document
---

```{r setup}
#Seguindo o procedimento Extração, Tratamento e Carga
...
>>>>>>> 890211f412885f27ad0077b7a72311bc5a4ebd9d
...
#Importar população - fonte IPARDES (pop estimada)
...
{
#renomear colunas
pop_pr <- rename(pop_pr, city = Localidade, variable = Vari�vel )
...
#Bases de profissionais da saúde e número de óbitos - fonte IPARDES
...
#Tranformar anos que estão nomeando colunas em dados de um coluna year
dprofissionaisobitos <- melt(dprofissionaisobitos)

dprofissionaisobitos <- rename(dprofissionaisobitos, city = Localidade, year = variable, variable = Vari�vel, result = value)
...
}#Profissionais de Saúde e obitos
...
#Retirar linhas em que o resultado é 0 ou NA
Hospitais <- Hospitais[!(is.na(Hospitais$value) | Hospitais$value==""), ]

#Renomear colunas
Hospitais <- rename(Hospitais, city = Localidade, year = variable, variable = Vari�vel, result = value)

#Conferir se há algum NA
Hospitais <- Hospitais[!is.na(Hospitais$result),]

#Inserir uma coluna para população
...
#Trocar de virgula para ponto para manipulação no R
...
dhealth <- rbind(dprofissionaisobitos, Hospitais)
  ...
  #Unir base saúde ao código correto de cidade
dhealth <- left_join(fcity, dhealth, by = "city")

  #Retirar colunas de cidade e região
dhealth <- dhealth[, -c(2:3)]

  #Renomear variáveis para melhor visualização do BI
  #Mortalidade Geral
...
dhealth$variable <- gsub("Hospitaliza??es pelo SUS [-] Geral por Local de Resid?ncia [-] Gastos Total com Interna??es [(]R[$] 1,00[)]", "Gastos Totais com Interna??oes", dhealth$variable, ignore.case =  TRUE)
  ...
  #Taxa de Mortalidade nas Internações
...
```{r export health}
  ...
  #Base crua (raw)
#Exportar Base Profissionais da Saúde e obitos
...
{
  #Renomear Variaveis
  AE <- rename(AE, total_elegiveis = total.de.pessoas.eleg�veis.ao.recebimento.do.aux�lio.emergencial)
AE <- rename(AE, soma_repasse = Soma.do.total.a.ser.repassado.pelo.Aux�lio.emergencial.e.pela.extens�o.do.aux�lio..todos.os.p�blicos.)
  ...
  AE_PR <- AE %>%
  filter(UF == "PR", Refer�ncia == 2020)
  
  #Renomear colunas
  AE_PR <- rename(AE_PR,code = C�digo, year = Refer�ncia, city = Unidade.Territorial)
